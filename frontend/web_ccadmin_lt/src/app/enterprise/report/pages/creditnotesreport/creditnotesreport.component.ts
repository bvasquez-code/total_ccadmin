...
@Component({
    selector: 'app-creditnotesreport',
    templateUrl: './creditnotesreport.component.html'
})
export class CreditNotesReportComponent implements OnInit, OnDestroy, ActionTableService<ReportRowDto> {
    readonly title = "Reporte de devoluciones y notas de crédito";
    readonly description = "Notas de crédito por fecha de creación, con motivo, importes y estado. Distingue las unidades acreditadas de las unidades aceptadas de vuelta al stock.";
    ...
    loadingTable(responsePageSearch: ResponsePageSearch<ReportRowDto>): void {
        ...
        data.init(
            [
                { Name: "Nota de crédito", key: 'CreditNoteCod', FunctionKey: (row: ReportRowDto) => row['CreditNoteCod'] ?? '—' },
                ...
                { Name: "Estado", key: 'CreditNoteStatus', IsStatus: true, Html: {}, Mask: {"P":"Pendiente","C":"Confirmado","X":"Anulado","R":"Rechazado","F":"Finalizado","":"—"}, FunctionKey: (row: ReportRowDto) => row['CreditNoteStatus'] ?? '' },
                { Name: "Motivo", key: 'Reason', FunctionKey: (row: ReportRowDto) => row['Reason'] ?? '—' },
                ...
                { Name: "Total acreditado", key: 'Amount', FunctionKey: (row: ReportRowDto) => amount(row['Amount']), CellClassFunction: () => 'text-right text-nowrap' },
                { Name: "Unidades acreditadas", key: 'Quantity', FunctionKey: (row: ReportRowDto) => number(row['Quantity']), CellClassFunction: () => 'text-right text-nowrap' },
                { Name: "Unidades devueltas", key: 'ReturnedQuantity', FunctionKey: (row: ReportRowDto) => number(row['ReturnedQuantity']), CellClassFunction: () => 'text-right text-nowrap' },
                { Name: "Stock procesado", key: 'IsStockReturned', IsStatus: true, Html: {}, Mask: {"S":"Sí","N":"No","":"—"}, FunctionKey: (row: ReportRowDto) => row['IsStockReturned'] ?? '' },
                { Name: "Pagada / aplicada", key: 'IsPaid', IsStatus: true, Html: {}, Mask: {"S":"Sí","N":"No","":"—"}, FunctionKey: (row: ReportRowDto) => row['IsPaid'] ?? '' }
            ],
    ...
    async findAll(Page: number, Query: string, filters: ReportFilterDto = this.filters): Promise<void> {
        ...
        this.errorMessage = '';
        try {
            const response = await this.reportService.findCreditNotes(search);
    ...
    async exportExcel(): Promise<void> {
        ...
        this.exporting = true;
        try {
            const response = await this.reportService.exportCreditNotes(filters);
            if (request !== this.requestNumber) return;
