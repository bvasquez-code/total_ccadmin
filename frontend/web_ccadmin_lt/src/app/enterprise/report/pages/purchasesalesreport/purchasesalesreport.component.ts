...
@Component({
    selector: 'app-purchasesalesreport',
    templateUrl: './purchasesalesreport.component.html',
    styleUrls: ['../../financial-report.css']
})
export class PurchaseSalesReportComponent implements OnInit, OnDestroy, ActionTableService<ReportRowDto> {
    readonly title = 'Compras vs. ventas por tienda';
    readonly description = 'Balance de los movimientos de mercadería de la tienda seleccionada o de todas las tiendas. Las entradas restan y las salidas suman al saldo: ventas y devoluciones al importe de venta; compras, transferencias y ajustes al costo trazado. Incluye movimientos de inventario, por lo que no representa cobros, pagos ni utilidad contable.';
    ...
    get appliedStoreName(): string {
        if (!this.appliedFilters?.StoreCod) return 'Todas las tiendas';
        const store = this.storeList.find(item => item.StoreCod === this.appliedFilters?.StoreCod);
        return store ? `${store.StoreCod} - ${store.Name}` : this.appliedFilters?.StoreCod || '';
    }

    currencyLabel(currencyCod: string): string {
        return currencyCod === 'UNKNOWN' ? 'Sin moneda' : currencyCod;
    }
    ...
    updateChart(): void {
        ...
        this.trend = new ReportTrendDto(this.overviewData.Daily,
            ...
            this.summaryCurrency, this.cumulative, 'COMPARISON');
    }
    ...
    loadingTable(responsePageSearch: ResponsePageSearch<ReportRowDto>): void {
        ...
        data.init(
            [
                ...
                { Name: 'Operación', key: 'SourceType', IsStatus: true,
                  Html: { SALE: 'text-success', PURCHASE: 'text-danger', CREDIT_NOTE: 'text-warning',
                      TRANSFER: 'text-primary', STOCK_ENTRY: 'text-info', STOCK_EXIT: 'text-info' },
                  Mask: { SALE: 'Venta', PURCHASE: 'Compra', CREDIT_NOTE: 'Nota de crédito',
                      TRANSFER: 'Transferencia', STOCK_ENTRY: 'Entrada de stock', STOCK_EXIT: 'Salida de stock' } },
                { Name: 'Movimiento', key: 'TypeOperation', IsStatus: true,
                  Html: { S: 'text-danger', R: 'text-success' }, Mask: { S: 'Entrada', R: 'Salida' } },
                { Name: 'Documento', key: 'OperationCod' },
                { Name: 'Referencia', key: 'Reference', FunctionKey: (row: ReportRowDto) => row['Reference'] || '—' },
                { Name: 'Local', key: 'StoreName' },
                { Name: 'Cliente / proveedor', key: 'PartnerCod', FunctionKey: (row: ReportRowDto) => row['PartnerCod'] || '—' },
                { Name: 'Unidades base', key: 'Quantity' },
                { Name: 'Moneda', key: 'CurrencyCod', Mask: { UNKNOWN: 'Sin moneda' },
                  FunctionKey: (row: ReportRowDto) => this.currencyLabel(String(row['CurrencyCod'])) },
                { Name: 'Salidas valorizadas (+)', key: 'Sales', FunctionKey: (row: ReportRowDto) => amount(row['Sales']), CellClassFunction: () => 'text-right' },
                { Name: 'Entradas valorizadas (−)', key: 'Purchases', FunctionKey: (row: ReportRowDto) => amount(row['Purchases']), CellClassFunction: () => 'text-right' },
                { Name: 'Efecto en el saldo', key: 'Balance', FunctionKey: (row: ReportRowDto) => amount(row['Balance']),
                  CellClassFunction: (row: ReportRowDto) => Number(row['Balance']) >= 0 ? 'text-right text-success font-weight-bold' : 'text-right text-danger font-weight-bold' }
            ],
    ...
    async findAll(Page: number, Query: string, filters: ReportFilterDto = this.filters): Promise<void> {
        ...
        try {
            const [response, overview] = await Promise.all([
                this.reportService.findPurchaseSales(search), this.reportService.overviewPurchaseSales(search)
            ]);
    ...
    async exportExcel(): Promise<void> {
        ...
        try {
            const [response, overview] = await Promise.all([
                this.reportService.exportPurchaseSales(filters), this.reportService.overviewPurchaseSales(filters)
            ]);
            if (request !== this.requestNumber) return;
