DROP PROCEDURE IF EXISTS `p_manage_credit_note_document`;

DELIMITER $$

CREATE PROCEDURE `p_manage_credit_note_document`()
BEGIN
    ...
    AND table_name = 'credit_note_document';
...
--
        ...
        SELECT 'Tabla credit_note_document creada desde cero.' AS Mensaje;
    ...
    ELSE
        ...
        SELECT 'Tabla credit_note_document ya existe. No se realizaron cambios estructurales.' AS Mensaje;
...
-- Ejecutar y limpiar
CALL `p_manage_credit_note_document`();
DROP PROCEDURE `p_manage_credit_note_document`;
