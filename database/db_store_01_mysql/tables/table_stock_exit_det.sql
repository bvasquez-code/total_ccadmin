DROP PROCEDURE IF EXISTS `p_manage_stock_exit_det`;

DELIMITER $$

CREATE PROCEDURE `p_manage_stock_exit_det`()
BEGIN
    ...
    WHERE table_schema = DATABASE()
      AND table_name = 'stock_exit_det';

    IF v_table_exists = 0 THEN
        -- =============================================
        -- CASO: LA TABLA NO EXISTE -> CREARLA COMPLETA
        -- =============================================
        CREATE TABLE `stock_exit_det` (
          `StockExitCod` varchar(16) NOT NULL COMMENT 'Codigo de cabecera de retiro excepcional',
          ...
          `OriginStockExitCod` varchar(16) DEFAULT NULL COMMENT 'Codigo del documento original para una linea de resolucion',
          ...
          PRIMARY KEY (`StockExitCod`,`ItemNumber`),
          KEY `idx_stock_exit_det_product` (`ProductCod`,`Variant`,`WarehouseCod`),
          KEY `idx_stock_exit_det_origin` (`OriginStockExitCod`,`OriginItemNumber`),
          KEY `idx_stock_exit_det_unavailable_reason` (`UnavailableReasonCode`),
          KEY `idx_stock_exit_det_resolved_in_reason` (`ResolvedInReasonCode`),
          KEY `idx_stock_exit_det_resolved_out_reason` (`ResolvedOutReasonCode`),
          KEY `idx_stock_exit_det_resolution_reason` (`ResolutionReasonCode`),
          KEY `idx_stock_exit_det_warehouse` (`WarehouseCod`),
          ...
          CONSTRAINT `chk_stock_exit_det_resolution_version` CHECK (`ResolutionVersion` >= 0),
          CONSTRAINT `chk_stock_exit_det_resolution_type` CHECK (
              `ResolutionType` IS NULL OR `ResolutionType` in (_utf8mb4'L',_utf8mb4'B',_utf8mb4'D',_utf8mb4'M')
          ),
          CONSTRAINT `chk_stock_exit_det_origin_pair` CHECK (
              (`OriginStockExitCod` IS NULL AND `OriginItemNumber` IS NULL)
              OR (`OriginStockExitCod` IS NOT NULL AND `OriginItemNumber` IS NOT NULL)
          ),
          CONSTRAINT `chk_stock_exit_det_status` CHECK (`Status` in (_utf8mb4'A',_utf8mb4'I'))
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
          COMMENT='Detalle de retiros excepcionales de stock y sus resoluciones';

        SELECT 'Tabla stock_exit_det creada desde cero.' AS Mensaje;
    ELSE
        -- =============================================
        ...
        IF NOT EXISTS (
            SELECT * FROM information_schema.table_constraints
            WHERE table_schema = DATABASE() AND table_name = 'stock_exit_det'
              AND constraint_name = 'chk_stock_exit_det_resolved_out_type'
        ) THEN
            ALTER TABLE `stock_exit_det`
                ADD CONSTRAINT `chk_stock_exit_det_resolved_out_type`
        ...
        IF NOT EXISTS (
            SELECT * FROM information_schema.table_constraints
            WHERE table_schema = DATABASE() AND table_name = 'stock_exit_det'
              AND constraint_name = 'chk_stock_exit_det_resolution_version'
        ) THEN
            ALTER TABLE `stock_exit_det`
...
-- Ejecutar y limpiar
CALL `p_manage_stock_exit_det`();
DROP PROCEDURE `p_manage_stock_exit_det`;
