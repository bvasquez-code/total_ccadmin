...
BEGIN
    ...
    IF v_table_exists = 0 THEN
        ...
        CREATE TABLE `stock_entry_det` (
          ...
          CONSTRAINT `chk_stock_entry_det_prices` CHECK (`NumUnitPrice` >= 0 AND `NumTotalPrice` >= 0),
    ...
    ELSE
        ...
        IF NOT EXISTS (
            ...
            WHERE table_schema = DATABASE() AND table_name = 'stock_entry_det'
              AND constraint_name = 'chk_stock_entry_det_prices'
        ) THEN
            ALTER TABLE `stock_entry_det`
                ADD CONSTRAINT `chk_stock_entry_det_prices`
