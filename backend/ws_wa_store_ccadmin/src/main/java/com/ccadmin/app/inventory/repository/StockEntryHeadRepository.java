...
public interface StockEntryHeadRepository extends JpaRepository<StockEntryHeadEntity, String> {
    @Query(value = "CALL db_store_01.get_cod_trx(:storeCod, 'stock_entry_head')", nativeQuery = true)
    String createCode(@Param("storeCod") String storeCod);

    @Query(value = "select * from stock_entry_head where StockEntryCod=:code for update", nativeQuery = true)
    StockEntryHeadEntity findForUpdate(@Param("code") String code);

    @Query(value = """
        select count(1) from stock_entry_head h
        where h.StoreCod=:storeCod and h.Status='A'
          and (:query='' or h.StockEntryCod like concat('%',:query,'%') or h.Observation like concat('%',:query,'%'))
    ...
    @Query(value = """
        select * from stock_entry_head h
        where h.StoreCod=:storeCod and h.Status='A'
          and (:query='' or h.StockEntryCod like concat('%',:query,'%') or h.Observation like concat('%',:query,'%'))
          and (:processStatus='' or h.ProcessStatus=:processStatus)
          and (:processType='' or h.ProcessType=:processType)
        ...
        """, nativeQuery = true)
    List<StockEntryHeadEntity> search(@Param("storeCod") String storeCod, @Param("query") String query,
                    @Param("processStatus") String processStatus, @Param("processType") String processType,
                    @Param("dateStart") String dateStart, @Param("dateEnd") String dateEnd,
