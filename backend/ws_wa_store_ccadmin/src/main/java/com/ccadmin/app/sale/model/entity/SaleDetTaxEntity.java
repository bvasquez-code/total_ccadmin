...
@Entity
@Table(name = "sale_det_tax")
@IdClass(SaleDetTaxID.class)
public class SaleDetTaxEntity extends AuditTableEntity implements Serializable {
    ...
    public SaleDetTaxEntity validate() {
        if (SaleCod == null || SaleCod.isBlank()) {
            throw new SaleBuildException("Codigo de venta esta vacio");
        }
        if (ItemNumber <= 0 || TaxLineNumber <= 0) {
            throw new SaleBuildException("Numero de linea tributaria no valido");
        }
        ...
        if (TaxRateValue.compareTo(BigDecimal.ZERO) < 0
            ...
            throw new SaleBuildException("Valores tributarios no pueden ser negativos");
        }
        return this;
    }

    @Override
    public SaleDetTaxEntity session(String userCod) {
